...
import me.ctknight.uploadmanager.UploadCancelException;
import me.ctknight.uploadmanager.UploadContract;
import me.ctknight.uploadmanager.UploadException;
...
class UploadThread implements Runnable, CountingRequestBody.Listener {
  ...
  public void onRequestProgress(long bytesWritten, long contentLength) {
    mMadeProgress = true;
    mInfoDelta.mCurrentBytes = bytesWritten;
    mInfoDelta.mTotalBytes = contentLength;
    try {
      updateProgress();
  ...
  private ParcelFileDescriptor getFileDescriptor() throws IOException {
    Uri fileUri = Uri.parse(mInfo.mFileUri);
    try {
      return mContext.getContentResolver().openFileDescriptor(fileUri, "r");
    } catch (IOException e) {
  ...
  private RequestBody buildRequestBody() throws IOException {
    MultipartBody.Builder builder = new MultipartBody.Builder()
        ...
        .addFormDataPart("file", mInfo.mFileName,
            OkHttpUtils.createRequestFromFile(
                MediaType.parse(mInfo.mMimeType), getFileDescriptor()));
    for (Map.Entry<String, String> cd : mInfo.getContentDisposition().entrySet()) {
      builder.addFormDataPart(cd.getKey(), cd.getValue());
    }
    RequestBody realBody = builder.build();
    try {
    ...
    RequestBody wrapper = new CountingRequestBody(realBody, this);
    return wrapper;
  }
